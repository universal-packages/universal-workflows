...
describe(Workflow, (): void => {
  it('stops all the strategy routines when one fails', async (): Promise<void> => {
    ...
    expect(listener.mock.calls).toContainEqual([
      { event: 'step:stopped', error: stepStopError, measurement, payload: { index: 0, routine: 'test1 [0]', strategy: 'test1', strategyIndex: 0 } }
    ])
    expect(listener.mock.calls).toContainEqual([
      { event: 'routine:stopped', error: routineStopError, measurement, payload: { name: 'test1 [0]', strategy: 'test1', strategyIndex: 0 } }
    ])
    ...
    expect(listener.mock.calls).toContainEqual([
      { event: 'step:stopped', error: stepStopError, measurement, payload: { index: 0, routine: 'test1 [1]', strategy: 'test1', strategyIndex: 1 } }
    ])
    expect(listener.mock.calls).toContainEqual([
      { event: 'routine:stopped', error: routineStopError, measurement, payload: { name: 'test1 [1]', strategy: 'test1', strategyIndex: 1 } }
    ])
    ...
    expect(listener.mock.calls).toContainEqual([
      { event: 'step:stopped', error: stepStopError, measurement, payload: { index: 0, routine: 'test1 [2]', strategy: 'test1', strategyIndex: 2 } }
    ])
    expect(listener.mock.calls).toContainEqual([
      { event: 'routine:stopped', error: routineStopError, measurement, payload: { name: 'test1 [2]', strategy: 'test1', strategyIndex: 2 } }
    ])
    ...
    expect(listener.mock.calls).toContainEqual([
      { event: 'step:output', payload: { index: 0, routine: 'test1 [4]', data: 'usage: sleep seconds\n', strategy: 'test1', strategyIndex: 4 } }
    ])
    expect(listener.mock.calls).toContainEqual([
      {
        event: 'step:failure',
    ...
    expect(listener.mock.calls).toContainEqual([{ event: 'failure', error: new Error('Workflow failed'), measurement }])
    expect(listener.mock.calls).toContainEqual([{ event: 'end', error: new Error('Workflow failed'), measurement, payload: { endedAt } }])
  })
})
