...
describe(Workflow, (): void => {
  it('continue running routines if a strategy fails if it was expected to', async (): Promise<void> => {
    ...
    expect(listener.mock.calls).toContainEqual([{ event: 'step:output', payload: { index: 0, routine: 'test3 [4]', data: 'Command failed', strategy: 'test3', strategyIndex: 4 } }])
    expect(listener.mock.calls).toContainEqual([
      {
        event: 'step:failure',
    ...
    expect(listener.mock.calls).toContainEqual([{ event: 'step:output', payload: { index: 0, routine: 'test5 [4]', data: 'Command failed', strategy: 'test5', strategyIndex: 4 } }])
    expect(listener.mock.calls).toContainEqual([
      {
        event: 'step:failure',
        error: new Error('Process exited with code 1\n\nCommand failed'),
